...
const CalculateMetrics = ({data}) => {
  ...
  return (
    <div className="mt-16  ml-[75px] py-5 px-12 w-full">
       {/*  <div className='mb-3'>
            <button className='flex items-center px-5 bg-indigo-500 py-2 rounded-md text-white'
            onClick={()=>addMetricCount()}
      ...
      {metricsCount.map((count, index)=>(
        <>
        <div key={index} className="flex flex-row gap-x-16">
         
        <div className="flex  bg-white rounded-lg w-[40%] h-[100px] shadow-lg">
        <Menu as="div">
    ...
    <Menu.Items className="absolute mt-1  top-left ml-[-10] bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none w-[300px] z-10">
      <div className="px-1 py-1 w-full ">
        ...
        <Menu as="div">
  <Menu.Button className="text-secondary mt-9 text-white px-5 py-1 rounded-[5px]">
     {selectedMetric2 ? selectedMetric2 : 'Select a Metric'}
  </Menu.Button>
  <Transition
    as={Fragment}
    enter="transition ease-out duration-100"
    ...
    <Menu.Items className="origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none w-[400px] z-10">
      <div className="px-1 py-1 w-full ">
        {finalData && finalData.map((item) => (
          <Menu.Item key={item?.name} className="z-10">
            {({ active }) => (
              <button
               ...
               onClick={()=>handleClick2(item?.name, item?.value)}
              >
                {item?.name}
              </button>
            )}
          ...
          {/* Result */}
          <div className="flex items-center bg-white rounded-lg w-[40%] h-[100px] ml-[368px] shadow-lg">
